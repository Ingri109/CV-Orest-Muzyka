...
export default function LocalSwitcher({ modal }: { modal: string }) {
        ...
        return (
            <>
                <div className="cursor-pointer mr-[10px] md:mr-[28px]" onClick={() => setOpen(!open)}>
                    <Image loading="lazy" src={World} alt={'world'} className="w-[20px] h-[20px] md:w-[24px] md:h-[24px]" />
                </div>
                <div ref={backdrop} className={` ${open ? 'block' : 'hidden'} z-30 absolute top-14 right-2 text-white bg-black border-[2px] border-color1 px-1.5 py-1.5 rounded-lg md:right-4`}>
                    <div className="cursor-pointer w-full flex justify-start items-center bg-transparent rounded-md px-3 py-1 hover:bg-[#393937]" onClick={() => onSelectChange('en')}>
                        {locale == 'en' ? <Image loading="lazy" src={Tick} alt="tick" className="w-[16px] h-[16px] mr-2"></Image> : <div className="w-[16px] h-[16px] mr-2"></div>}
                    ...
                    <div className="cursor-pointer w-full flex justify-start items-center bg-transparent rounded-md px-3 py-1 hover:bg-[#393937]" onClick={() => onSelectChange('pl')}>
                        {locale == 'pl' ? <Image loading="lazy" src={Tick} alt="tick" className="w-[16px] h-[16px] mr-2"></Image> : <div className="w-[16px] h-[16px] mr-2"></div>}
                    ...
                    <div className="cursor-pointer w-full flex justify-start items-center bg-transparent rounded-md px-3 py-1 hover:bg-[#393937]" onClick={() => onSelectChange('ua')}>
                        {locale == 'ua' ? <Image loading="lazy" src={Tick} alt="tick" className="w-[16px] h-[16px] mr-2"></Image> : <div className="w-[16px] h-[16px] mr-2"></div>}
    ...
    if (modal === 'NavBar') {
        return (
            <>
                <div className="flex flex-col h-[160px] relative items-center justify-end justify-items-stretch mt-20">
                    <div className={` ${open ? 'block' : 'hidden'} w-auto text-white mb-2 bg-black border-[2px] border-color1 px-2.5 py-1.5 rounded-lg `}>
                        <div className="cursor-pointer w-full flex justify-start items-center bg-transparent rounded-md px-3 py-1 hover:bg-[#393937]" onClick={() => onSelectChange('en')}>
                    ...
                    <button className="bg-[#202020] w-auto rounded-lg border border-color1 px-10 py-1.5 text-white text-[18px] font-semibold " onClick={() => setOpen(!open)} type="button">{language}</button>
                </div>
            </>
        )
    }
